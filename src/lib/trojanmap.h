...
class TrojanMap {
 public:
  ...
  std::vector<std::vector<std::string>> queries_path;
...
private:
   ...
   void dfsHelper( trieNode*, std::string, std::vector<std::string>&);

   bool cycleHelper( std::unordered_map<std::string, bool>&, std::unordered_map<std::string, bool>&, std::vector<double> &, std::string, std::string );

   std::vector<std::string> queryHelper( std::string& name1, std::string& name2, double& gas );
   void backtrackingHelper( std::vector<int> cur, double& min_dis, double cur_dis, std::pair<double, std::vector<std::vector<int>>>& results, std::vector<bool> visited, std::vector<std::string> location_ids );
