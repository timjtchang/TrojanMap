...
void MapUI::PrintMenu() {
  ...
  case 5:
  {
    ...
    menu = "Please input the regular expression of a location's name:";
    std::cout << menu;
    std::string reg;
    getline(std::cin, reg);
    ...
    try {
        std::regex re(reg);
        output=map.GetLocationRegex(re);

    }
    catch (const std::regex_error& e){

      std::cout<<"Regex is invalid "<<std::endl;
    } 
 ...
 */
void MapUI::PlotPaths(std::vector<std::vector<std::string>> &path) {
  std::string image_path = cv::samples::findFile("src/lib/map.png");
  cv::Mat img = cv::imread(image_path, cv::IMREAD_COLOR);

  for( const auto& location_ids:path ){
...
void MapUI::DynamicPrintMenu() {
  ...
  std::string number_str(number_ch);
  ...
  case 2:
  {
    ...
    if (results.first != -1) {
      ...
      menu = "**************************************************************\n";
      y = ui.ScrollLongText(menu, 10, y);
      y = ui.ScrollLongText("Did you mean " + tmp + " instead of " + input + "? [y/n]", 10, y);
      scanw("%s",input);
      std::string tmp_y = "y";
  ...
  case 4:
  {
    ...
    if( output[0] != "(-1,-1)" ){
      ...
      for (auto x: output) {
        ...
        y=ui.ScrollLongText("ID: \""+x+"\""+" -- "+" Name: \""+name+"\""+", ",10,y);
  ...
  case 5:
  {
    ...
    try {
      ...
      y = ui.ScrollLongText( "Regex is invalid ", 10, y);
  ...
  case 6:
  {
    ...
    if (results.size() != 0) {
      // for (auto x : results) y = ui.ScrollLongText(x, 10, y);
      y = ui.ScrollLongText("The distance of the path is:" + std::to_string(map.CalculatePathLength(results)) + " miles\n", 10, y);
      PlotPath(results);
    } else {
      y = ui.ScrollLongText("No route from the start point to the destination.\n", 10, y);
    }
    menu = "**************************************************************\n";
    y=ui.ScrollLongText(menu,10,y);
    y=ui.ScrollLongText("Time taken by function: " + std::to_string(duration.count()/1000) + " ms\n",10,y);
  ...
  case 8:
  {
    ...
    y = ui.ScrollLongText( "Please input the locations filename:", 10, y);
    scanw("%s",input);
    std::string locations_filename(input);

    y = ui.ScrollLongText( "Please input the dependencies filename:", 10, y);
    scanw("%s",input);
  ...
  case 9:
  {
    ...
    if (results.second.size() != 0) {
      ...
      for( int i=0 ; i<results.second[results.second.size()-1].size() ; i++ ){

        str = str+"\"" + results.second[results.second.size()-1][i] + "\",";
    ...
    case 10:
  {
    ...
    menu = "*************************Results******************************";
    y=ui.ScrollLongText( menu, 10, y);
    y=ui.ScrollLongText( "Find Nearby Results:\n", 10, y);

    int cnt = 1;
    for (auto x : result) { 

      y=ui.ScrollLongText( std::to_string(cnt) + " " + map.data[x].name + "\n", 10, y);
      cnt++;
    }
    PlotPointsLabel(result, map.GetID(origin));
    menu = "**************************************************************\n";
  ...
  case 11:
  {
    ...
    if (results.size() != 0) {
      ...
      y=ui.ScrollLongText( "\nThe distance of the path is:" + std::to_string (map.CalculatePathLength(results)) + " miles\n", 10, y);
      PlotPathwithName(results);
    } else {
  ...
  case 12:
  {
    ...
    while (true) {
      ...
      y=ui.ScrollLongText( menu, 10, y);
      scanw("%s",input);
      std::string input3(input);
      double volumn = std::stod(input3);
      Q.push_back({volumn, {input1, input2}});
      menu = "More Query? (y/n)";
      y=ui.ScrollLongText( menu, 10, y);
      scanw("%s",input);
      std::string input4(input);
      if (input4 != "y") break;
    }
    auto start = std::chrono::high_resolution_clock::now();
    auto results = map.Queries(Q);
    ...
    y=ui.ScrollLongText( menu, 10, y);
    if (results.size() != 0) {
      for (int i = 0; i < Q.size(); i++) y=ui.ScrollLongText(  "From " + Q[i].second[0] + " to " + Q[i].second[1] + " with " + std::to_string (Q[i].first) + " gallons of gas tank: " + (results[i] ? "Yes" : "No") + "\n", 10, y);
